...
const float min_voltage = 1.2; // geringste Eingangsspannung der Drehzahlregler (in V)
const float max_voltage = 2.6; // höchste Eingangsspannung der Drehzahlregler (in V)
...
const float i = 10.8/4.91; // Übersetzungsverhältnis der Motoren
...
const uint16_t max_poti = 350; // Potentiometerstellung unter Zug
...
const float max_steps = (422 / 25) * steps_per_revolution; // Länge der Linearführung in Schritten (l = 422 mm)
...
void Set_Zyl_Speed(int16_t new_speed) {
  ...
  // Umwandlung des speed-Werts in einen dac-Wert:
  dac_value = map(abs(current_speed), 0, 1000, (int)dac_min_voltage, (int)dac_max_voltage);

  // Ausgabe der speed-Werte an den Motor:
...
void Stepper_Interrupt() {
  ...
  // regulärer Betrieb des Schrittmotors:
